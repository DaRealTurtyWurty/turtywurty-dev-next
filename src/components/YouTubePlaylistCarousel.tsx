...
export default async function YouTubePlaylistCarousel({
    ...
    const content = (
        <>
            {!embedded && <h2 className="text-3xl font-bold mb-6 text-center">{title}</h2>}
            <section className={`${embedded ? "" : "bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-12"}`}>
                <div className={`mb-5 ${embedded ? "" : "flex items-center justify-between gap-3"}`}>
                    <div className={embedded ? "" : "flex-1"}>
                        <h3 className={`${embedded ? "text-2xl font-semibold" : "sr-only"}`}>{title}</h3>
                        <p className={`text-gray-600 dark:text-gray-300 ${embedded ? "mt-2" : ""}`}>
                            {description}
                        </p>
                    </div>
